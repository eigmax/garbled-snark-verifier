...
use crate::blake3::blake3_utils::{compress, get_flags_for_block, TablesVars};
...
mod tests {
    ...
    fn test_blake3() {
        ...
        for limb_len in ALL_POSSIBLE_LIMB_LENGTHS {
            let script = blake3_compute_script_with_limb(message.len(), limb_len);
            let optimized_script = optimizer::optimize(script.clone().compile());
            println!("[{:?}] script: {:?}, optimized: {:?}", limb_len, script.len(), optimized_script.len());
            let run_script = script! {
                { blake3_push_message_script_with_limb(&message, limb_len) }
                { script }
                { blake3_verify_output_script(expected_hash) }
            };
            assert!(execute_script_without_stack_limit(run_script).success);
        }
    }
}
